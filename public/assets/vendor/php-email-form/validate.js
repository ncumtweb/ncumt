...
function caculate() {
    ...
    var trip_tag = document.getElementById("trip_tag").value;
    ...
    if (check(name, normal_day, abnormal_day, level, road, terrain, plant, energy, water)) {
        if (document.getElementById("store_result") != null) {
        ...
        score = Number(caculateTripTag(score, trip_tag));
...
function caculateTripTag(score, trip_tag) {
    if (Number(trip_tag) == 1) {
        score = score * 1.1;
    }
    else if (Number(trip_tag) == 2) {
        score = score * 0.9;
    }
