                                    ...
                                    <div id="faq-content-{{ $faq->id }}" class="accordion-collapse collapse"
                                        ...
                                        <div class="accordion-body">
                                            <strong>
                                                ...
                                                @auth
                                                    @if(Auth::user()->role > 0)
                                                        @if(empty($faq->answer))<!-- 如果回答為空 -->
                                                        <form action="{{ route('faq.edit', $faq->id) }}" method="POST">
                                                            <button type="button" class="far fa-square"
                                                                    onclick="window.location='{{ route('faq.edit', $faq->id) }}'">
                                                                回答問題
                                                            </button>
                                                        </form>

                                                        @else <!-- 如果回答不為空，顯示編輯和刪除按鈕 -->
                                                        <form action="{{ route('faq.destroy', $faq->id) }}" method="POST">
                                                            <button type="button" class="bi bi-pencil-square"
                                                                    onclick="window.location='{{ route('faq.edit', $faq->id) }}'"></button>
                                                            @csrf
                                                            @method('DELETE')
